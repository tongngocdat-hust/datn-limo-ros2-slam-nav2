...
mkdir -p "${BAGS_DIR}"
BAG_NAME="${SYSTEM}_${SCENARIO}_run${RUN_ID}_imu_${IMU}"
BAG_PATH="${BAGS_DIR}/${BAG_NAME}"

if [[ -e "${BAG_PATH}" ]]; then
    echo "Bag already exists: ${BAG_PATH}" >&2
    echo "Choose another --run value or move the old bag manually." >&2
    exit 1
fi

echo "Current ROS 2 topics:"
ros2 topic list || true

if ! ros2 topic list | grep -Fxq "/map"; then
    echo "WARNING: /map topic is not currently available. Recording will continue." >&2
fi
